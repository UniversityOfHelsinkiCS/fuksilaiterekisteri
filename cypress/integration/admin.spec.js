...
context('Admin', () => {
  ...
  it('Redirects admin to the correct page', () => {
    cy.contains('admin')
    cy.contains('non-fuksiEtunimi').parent().parent().find('td:contains(Kyllä)').should('have.length', 2)
  })
  ...
  it('Can toggle user roles', () => {
    ...
    cy.contains('non-fuksiEtunimi').parent().parent().find('td:contains(Kyllä)').should('have.length', 4)
