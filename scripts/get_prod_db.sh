...
CONTAINER=fuksilaiterekisteri_db
SERVICE_NAME=db
DB_NAME=postgres

FUKSILAITE_FILE_NAME=fuksilaite.sql.gz

SERVER=svm-116.cs.helsinki.fi
SERVER_PATH=/home/toska_user/most_recent_backup_store/

SERVER_FILE=${SERVER_PATH}${FUKSILAITE_FILE_NAME}
...
echo "Populating Fuksilaite"
docker exec -i $CONTAINER /bin/bash -c "gunzip | psql -U postgres" < ${BACKUPS}${FUKSILAITE_FILE_NAME}
